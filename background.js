...
chrome.runtime.onMessage.addListener(
    function(message, sender, sendResponse) {
        ...
        // Message Type:        URL Request
        // Description:         A Page (content script) is requesting its current URL
        // Response Attributes:
        //     - url:           A Promise that will eventually contain the page's URL
        if (message.MESSAGE_TYPE === "URL_REQ") {
            sendResponse({url: new Promise(function(resolve, reject) {
        ...
        if (changeInfo.url.toLowerCase().includes("pie.iu.edu/inventoryreports")) {
            ...
            chrome.history.search({text: '', maxResults: 2}, function(data) {
                if (data.length > 1) {
                    ...
                    if (!(data[1].url.toLowerCase().includes("pie.iu.edu/inventoryreports"))) {
